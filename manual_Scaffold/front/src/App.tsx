...
import ZKPassportVerification from "./components/ZKPassportVerification";
import "./App.css";
...
function ScaffoldApp() {
  ...
  const fetchContractState = async (contractName: string) => {
    try {
      const response = await fetch(
        ...
        }/v1/indexer/contract/${contractName}/state`
      );
    ...
    while (attempts < maxAttempts) {
      try {
        const response = await fetch(
          ...
          }/v1/indexer/transaction/hash/${txHash}`
        );
        if (!response.ok) {
  ...
  const handleZkVerificationComplete = (
    verified: boolean,
    uniqueIdentifier: string,
    proofData: any[]
  ) => {
    setZkPassportVerified(verified);
    if (verified) {
      setInitialResult(
        `ZK Passport verification successful! Unique ID: ${uniqueIdentifier}`
      );
    }
    console.log("ZK Verification completed:", {
      verified,
      uniqueIdentifier,
      proofData,
    });
  ...
  return (
    <div className="App">
      ...
      {showZkPassport && (
        <div className="zkpassport-section">
          <ZKPassportVerification
            onVerificationComplete={handleZkVerificationComplete}
          />
        </div>
      )}
